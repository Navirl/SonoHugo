...
export const createNote = async (
  ...
  if (path) {
    try {
      ...
      const newFile = await app.vault.getAbstractFileByPath(`${path[1]}.md`) as TFile;
      if(newFile === null) throw Error("Filepath does not exist!");
      await app.workspace.getLeaf().openFile(newFile);
      if (templateFile) {
        if (templater) {
          (app as any).plugins.plugins["templater-obsidian"].templater.append_template_to_active_file(templateFile);
        } else {
          (app as any).internalPlugins?.plugins["templates"].instance.insertTemplate(templateFile);
        }
      }
    } catch (e) {
