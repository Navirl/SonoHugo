...
export const template = async (
  ...
  if (templatesEnabled || templaterPluginEnabled) {
    ...
    // }
    const folder = args.tfolder || 
      args.templater && app.plugins?.plugins["templater-obsidian"]?.settings.templates_folder?.toLowerCase() || 
    ...
    //   return found;
    // })[0];
    const file = await app.vault.getAbstractFileByPath(`${folder}/${templateFile}.md`) as TFile;
    if (file) {
